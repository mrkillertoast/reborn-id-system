...
export const generateNewNumber = async () => {
  const dbItems: IIdNumbersCollection[] = await Database.fetchAllData(
    collection
  );
...
export const insertId = async (idNumber: number, char_id: string) => {
  const res = Database.insertData(
    { uid: idNumber, character_id: char_id },
    collection,
    true
  );
...
export const initCityHall = () => {
  Athena.controllers.marker.append({
    type: 1,
    pos: IDSystemConfig.CONFIG.CITYHALL_LOCATION,
  ...
  Athena.controllers.interaction.append({
    position: IDSystemConfig.CONFIG.CITYHALL_LOCATION,
    isPlayerOnly: true,
    isVehicleOnly: false,
    height: 3,
    ...
    callback(player: alt.Player) {
      alt.emitClient(
        player,
        RebornIdSystemEvents.ServerClient.OPEN_CITYHALL_WEBVIEW
      );
    },
  });
};
...
const createNewIdCard = async (player: alt.Player) => {
  ...
  if (typeof character === 'undefined') {
    Athena.player.emit.notification(
      player,
      IDSystemConfig.NOTIFICATION.USER_NOT_FULLY_LOGGED_IN
    );
    return;
  }
  ...
  if (!didAddItems) {
    Athena.systems.notification.toPlayer(
      player,
      IDSystemConfig.NOTIFICATION.GENERATING_FAILED
    );
    return;
  }
...
const showIdCard = async (player: alt.Player, ...args) => {
  if (args[2] === RebornIdSystemEvents.ServerClient.OPEN_CITYHALL_WEBVIEW) {
    ...
    if (!idCard) {
      Athena.systems.notification.toPlayer(
        player,
        IDSystemConfig.NOTIFICATION.PLAYER_NOT_FOUND
      );
      return;
    }
    ...
    alt.setTimeout(() => {
      alt.emitClient(
        player,
        RebornIdSystemEvents.ServerClient.OPEN_ID_WEBVIEW,
        idCard.data
      );
    }, 500);
  }
...
alt.onClient(
  RebornIdSystemEvents.WebViewToServer.GENERATE_NEW_ID,
  createNewIdCard
);
